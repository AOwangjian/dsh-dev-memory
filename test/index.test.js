...
test('queued user message preempts an active write-pass without stopping plugin followups', async () => {
  ...
  const agent = {
    ...
    send(message, target, wakeup) {
      if (nextTurn.some((row) => row.id === message.id)) throw new Error('message "' + message.id + '" is already pending');
      wakes.push({ message, target, wakeup });
    },
    cancel(cause, options) { cancels.push({ cause, options }); },
    inbox: {
      hasPending: true,
      nextTurn,
      remove(id) {
        const index = nextTurn.findIndex((row) => row.id === id);
  ...
  assert.equal(wakes[0].message.id, 'user-1');
});
...
test('ctx.inject(["webServer"]) mounts state and config routes', () => {
  ...
  assert.deepEqual(routes.map((r) => r.path).sort(), ['/dsh-dev-memory/config', '/dsh-dev-memory/jev/test', '/dsh-dev-memory/open', '/dsh-dev-memory/session-auto-write', '/dsh-dev-memory/state', '/dsh-dev-memory/workspaces']);
});
