...
export function makeMemoryService({ memoryRoot, scriptsDir, node = 'node', run = runScript }) {
  ...
  const resolveRoot = typeof memoryRoot === 'function' ? memoryRoot : () => memoryRoot;
  return {
    search(query, top = 5) {
      const root = resolveRoot();
      return JSON.parse(run(node, s(searchScript), [root, query, '--top', String(top), '--json']));
    },
    write(draft) {
      const root = resolveRoot();
      const abs = join(root, draft.relPath);
      const action = existsSync(abs) ? 'update' : 'create';
      mkdirSync(dirname(abs), { recursive: true });
      writeFileSync(abs, draft.content, 'utf8');

      let index = null;
      try {
        index = JSON.parse(run(node, s(crudScript), ['index-sync', root, '--json']));
      } catch (err) {
        ...
        if (err && err.status === 1) return { written: abs, action, index: null };
        throw err;
      }
      return { written: abs, action, index };
    },
    health() {
      const root = resolveRoot();
      return JSON.parse(run(node, s(healthScript), [root, '--json']));
    },
  };
}
