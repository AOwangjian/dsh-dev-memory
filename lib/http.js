...
/**
 * Register GET /dsh-dev-memory/state and POST /dsh-dev-memory/config.
 * @param webServer - DSH web server service (host.webServer).
 * @param handlers.getSnapshot - () => state payload
 * @param handlers.updateConfig - (body) => effective config
 */
export function mountDevMemoryRoutes(webServer, { getSnapshot, updateConfig }) {
  const disposers = [
    webServer.register({
      kind: 'exact',
      ...
      handler: (request, response) => {
        ...
        try {
          ...
          sendJson(response, 500, { error: err && err.message ? err.message : String(err) });
        }
      },
    }),
    webServer.register({
