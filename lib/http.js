 ...
 */
export function mountDevMemoryRoutes(webServer, { getSnapshot, updateConfig, listWorkspaces, mutateWorkspace, openPath, getSessionAutoWrite, setSessionAutoWrite, testJevConnection }) {
  const disposers = [
    webServer.register({
      kind: 'exact',
    ...
    webServer.register({
      kind: 'exact',
      path: '/dsh-dev-memory/jev/test',
      handler: async (request, response) => {
        if (!requireTrustedPost(request, response)) return;
        try {
          const result = typeof testJevConnection === 'function'
            ? await testJevConnection()
            : { ok: false, error: { category: 'unavailable', message: 'Jev connectivity test is unavailable' } };
    ...
    webServer.register({
      kind: 'exact',
      path: '/dsh-dev-memory/session-auto-write',
      handler: async (request, response) => {
        if (request.method === 'GET') {
          try {
            if (typeof getSessionAutoWrite !== 'function') {
              sendJson(response, 200, { sessionId: null, autoWrite: true, inherited: true });
              return;
            }
            sendJson(response, 200, getSessionAutoWrite(request));
          } catch (err) {
            sendError(response, err);
          }
          return;
        ...
        try {
          if (typeof setSessionAutoWrite !== 'function') {
            sendJson(response, 200, { ok: true, session: null });
            return;
          }
          const session = setSessionAutoWrite(body);
    ...
    webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          const result = typeof openPath === 'function' ? openPath(body) : null;
          sendJson(response, 200, { ok: true, ...(result && typeof result === 'object' ? result : { path: body && body.path }) });
        } catch (err) {
          sendError(response, err);
        }
      },
