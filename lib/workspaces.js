...
function slugOf(value) { return String(value).replace(/[:\\/_]/g, '-'); }
...
export function makeWorkspaceRegistry({ registryPath, projectsRoot, profile, now = Date.now, scanRoots, memoryRootFor } = {}) {
  if (!registryPath || !projectsRoot) throw new TypeError('registryPath and projectsRoot are required');
  const roots = Array.isArray(scanRoots) && scanRoots.length ? scanRoots.filter(Boolean) : [projectsRoot];
  const resolveMemoryRoot = typeof memoryRootFor === 'function'
    ? memoryRootFor
    : (_workspacePath, id) => join(projectsRoot, id, 'memory');
  ...
  function upsertVerified(workspacePath) {
    ...
    const rows = transaction(data => { mergeRecord(data, normalize({ id, name: workspaceName(path), workspacePath: path,
      memoryRoot: resolveMemoryRoot(path, id), verified: true, pinned: false, firstSeenAt: timestamp,
  ...
  return {
      ...
      return transaction(data => {
        for (const root of roots) {
          ...
          for (const entry of readdirSync(root, { withFileTypes: true })) {
            const memoryRoot = join(root, entry.name, 'memory');
    ...
    mutate(command = {}) {
      ...
      transaction(data => {
        ...
        if (command.action === 'unhide') { data.workspaces[index].hidden = false; result = normalize(data.workspaces[index]); return data; }
