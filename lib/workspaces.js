...
export function makeWorkspaceRegistry({ registryPath, projectsRoot, profile, now = Date.now }) {
  ...
  function release() { rmSync(lockPath, { recursive: true, force: true }); }
  ...
  function readLatest() {
    ...
    try {
      ...
      const stamp = now();
      copyFileSync(registryPath, join(dirname(registryPath), `workspaces.corrupt.${stamp}.json`));
  ...
  function transaction(change = x => x) {
    ...
    try {
      ...
      atomicWrite(next);
      return sorted(next.workspaces);
    } finally { release(); }
  }
  function ensureFile() {
    if (!existsSync(registryPath)) return transaction();
    const data = readLatest();
    if (data.workspaces.length === 0) return transaction(() => data);
    return sorted(data.workspaces);
  }
  function mergeRecord(data, incoming) {
    ...
    if (index < 0) data.workspaces.push(incoming);
      ...
      data.workspaces[index] = { ...old, ...incoming,
        ...
        lastSeenAt: discoveryCannotDowngrade ? old.lastSeenAt : incoming.lastSeenAt,
  ...
  return {
    ...
    mutate(command = {}) {
      ...
      transaction(data => {
        ...
        if (command.action === 'remove') { data.workspaces.splice(index, 1); return data; }
