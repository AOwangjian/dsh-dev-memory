...
export async function runWritePass({ proposal, service, auditPath, sessionId, autoWriteLevels, writeConfidenceMin }) {
  ...
  const entry = { sessionId, module: proposal.module, category: proposal.category, confidence: proposal.confidence, evidenceSource: proposal.evidence[0], action: 'write', relPath: proposal.draft && proposal.draft.relPath ? proposal.draft.relPath : null, summary: summarize(proposal) };
