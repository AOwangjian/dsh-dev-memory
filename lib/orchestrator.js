...
export async function runWritePass({ proposal, service, auditPath, sessionId, autoWriteLevels, writeConfidenceMin, confirmLevel1 = false }) {
  const v = validateProposal(proposal, { writeConfidenceMin });
  if (!v.accept) return { written: false, reason: v.reason };
  if (proposal.moduleLevel === 1 && !confirmLevel1) return { written: false, needsConfirm: true, reason: 'Level 1 需人工确认' };
  if (!confirmLevel1 && Array.isArray(autoWriteLevels) && proposal.moduleLevel != null && !autoWriteLevels.includes(proposal.moduleLevel)) {
