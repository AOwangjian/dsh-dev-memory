...
function applyInner(ctx, config = {}) {
  ...
  const stopWritePass = (id) => {
    const current = writePassState(id);
    if (current.agent && typeof current.agent.cancel === 'function') {
      try { current.agent.cancel({ kind: 'user' }, { keepInbox: true }); } catch { /* cancel is best-effort */ }
    }
    writePassByAgent.set(id, { queued: false, fromWritePass: false, skipNextIdle: true, agent: current.agent || null });
