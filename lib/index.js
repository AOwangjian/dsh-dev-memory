...
function defaultSkillScriptsDir() {
...
export function resolveScriptsDir({ configured, skillDir, bundledDir } = {}) {
  ...
  const skill = typeof skillDir === 'string' && skillDir.trim() ? skillDir.trim() : defaultSkillScriptsDir();
  if (hasMemoryScripts(skill)) return skill;
  ...
  return configuredDir || skill || bundled;
}
...
function applyInner(ctx, config = {}) {
  ...
  const state = {
    autoWrite: pluginConfig.read(bootAutoWrite).autoWrite,
    workspacePath: null,
