...
function applyInner(ctx, config = {}) {
  ...
  ctx.on(contract.EVENTS.AGENT_STATUS, ({ agent, status } = {}) => {
    if (!autoWriteForAgent(agent) || !agent) return;
    ...
    ctx.inject(['webServer'], (host) => {
      ...
      const mount = () => mountDevMemoryRoutes(host.webServer, {
        ...
        getSessionAutoWrite(request) {
          ...
          return sessionAutoWrite.resolve(sessionId, state.autoWrite);
        },
        setSessionAutoWrite(body) {
          ...
          return sessionAutoWrite.set(sessionId, payload.autoWrite);
        },
        listWorkspaces() {
