...
export function apply(ctx, config = {}) {
  ...
  const state = {
    ...
    workspacePath: resolveWorkspacePath(ctx),
  ...
  const bindAgentWorkspace = (agent) => {
    ...
    if (typeof cwd === 'string' && cwd.trim()) state.workspacePath = cwd.trim();
  };
  const effectiveRoot = () => state.memoryRootOverride || configuredRoot || deriveRoot(state.workspacePath);
  const service = makeMemoryService({ memoryRoot: effectiveRoot, scriptsDir });
  const effectiveConfig = () => ({
    memoryRoot: effectiveRoot(),
    workspacePath: state.workspacePath,
    rootMode: state.memoryRootOverride || configuredRoot ? 'configured' : 'auto',
    scriptsDir,
