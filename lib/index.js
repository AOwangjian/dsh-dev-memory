...
import { makeSessionAutoWriteStore } from './session-auto-write.js';
import { makePluginConfigStore } from './plugin-config.js';
import { DECISION_POLICY_SECTION, isDecisionPolicyEnabled } from './decision-policy.js';
import { makeJevService } from './jev-service.js';
import { makeJevTool } from './jev-tool.js';
import { gateHealth, gateRetrieval, gateWrite } from './decision-gates.js';
...
export function defaultDshProjectsRoot() {
  return join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'dev-memory', 'projects');
}

export function deriveRoot(workspace, opts = {}) {
  const slug = slugOf(workspace);
  const claudeRoot = typeof opts.claudeRoot === 'string' && opts.claudeRoot.trim() ? opts.claudeRoot.trim() : defaultClaudeProjectsRoot();
  const dshRoot = typeof opts.dshRoot === 'string' && opts.dshRoot.trim() ? opts.dshRoot.trim() : defaultDshProjectsRoot();
  const claudeMemory = join(claudeRoot, slug, 'memory');
  try {
    if (existsSync(claudeMemory) && statSync(claudeMemory).isDirectory()) return claudeMemory;
  } catch {
    /* missing Claude library is the portable default */
  }
  return join(dshRoot, slug, 'memory');
}

const REQUIRED_SCRIPTS = ['search-memory.mjs', 'memory-crud.mjs', 'health-check.mjs', 'utils.mjs'];

export function bundledScriptsDir() {
  return join(dirname(fileURLToPath(import.meta.url)), '..', 'scripts');
}

export function hasMemoryScripts(dir) {
  ...
  return REQUIRED_SCRIPTS.every((name) => existsSync(join(dir, name)));
}

export function resolveScriptsDir({ configured, bundledDir } = {}) {
  const configuredDir = typeof configured === 'string' ? configured.trim() : '';
  if (configuredDir && hasMemoryScripts(configuredDir)) return configuredDir;
  const bundled = typeof bundledDir === 'string' && bundledDir.trim() ? bundledDir.trim() : bundledScriptsDir();
  if (hasMemoryScripts(bundled)) return bundled;
  return configuredDir || bundled;
}
...
function defaultSessionAutoWritePath() {
  return join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'dev-memory', 'session-auto-write.json');
}

function defaultPluginConfigPath() {
  return join(process.env.DSH_HOME || join(homedir(), '.dsh'), 'dev-memory', 'config.json');
}

function defaultProjectsRoot() {
  return defaultDshProjectsRoot();
}
...
function jevAuditPath(root) {
  return join(root, '.audit', 'jev.jsonl');
}
...
const WRITE_PASS_INSTRUCTION = [
  'Development memory: at a goal-completion or session-end boundary, record only',
  'durable development memory. Prefer to update the existing topic file.',
  'Do not create a sibling file for a later stage of the same topic.',
  'Do not call `memory_write` mid-task, and do not call it when nothing durable',
  'was learned.',
].join(' ');

const BOUNDARY_REMINDER = [
  'You just reached a goal or session boundary. Review the work for durable',
  'development memory: a new module, a non-obvious decision, a recurring pitfall,',
  'or an open question. If there is nothing durable, stop and say that no memory',
  'update is needed. If there is, update the existing topic file by calling',
  '`memory_write`. Do not call `jev_decide` before a proposal exists.',
].join(' ');
...
// DSH projects this into ToolResultNode.meta independently of text rendering.
function memoryPresentationMeta(_args, value) {
  return { devMemory: value };
}

function healthRender(_args, value) {
  const summary = value && value.summary || {};
  const severity = summary.severityCounts || {};
  const decision = value && value.decision || {};
  const issues = value && value.issues || {};
  const issueRows = Object.entries(issues)
    .map(([name, rows]) => `| ${name} | ${Array.isArray(rows) ? rows.length : 0} |`)
    .join('\n') || '| 无 | 0 |';
  const attention = Number.isFinite(decision.needsAttention) ? decision.needsAttention.toFixed(2) : '—';
  return [{
    type: 'text',
    text: [
      '## 记忆健康检查',
      ...
      '| 指标 | 数值 |',
      '| --- | ---: |',
      `| Markdown 文件 | ${summary.markdownFiles || 0} |`,
      `| 目录 | ${summary.directories || 0} |`,
      `| README | ${summary.readmes || 0} |`,
      `| 严重度（高/中/低） | ${(severity.high || 0)}/${(severity.medium || 0)}/${(severity.low || 0)} |`,
      `| Jev needsAttention | ${attention} |`,
      '',
      '### 问题分类',
      '',
      '| 类别 | 数量 |',
...
function applyInner(ctx, config = {}) {
  ...
  const cfg = config || {};
  const configuredJev = cfg.jev && typeof cfg.jev === 'object' && !Array.isArray(cfg.jev) ? cfg.jev : {};
  const bootJev = {
    enabled: configuredJev.enabled === true,
    policyInjectionEnabled: !(configuredJev.policyInjection && configuredJev.policyInjection.enabled === false),
    apiKeyEnv: typeof configuredJev.apiKeyEnv === 'string' && configuredJev.apiKeyEnv.trim() ? configuredJev.apiKeyEnv.trim() : 'TYPESAFE_API_KEY',
    model: typeof configuredJev.model === 'string' && configuredJev.model.trim() ? configuredJev.model.trim() : 'jev-latest',
    gates: { retrieval: true, injection: true, write: true, health: true },
  };
  let configuredRoot = typeof cfg.memoryRoot === 'string' && cfg.memoryRoot.trim() ? cfg.memoryRoot.trim() : null;
  const scriptsDir = resolveScriptsDir({ configured: cfg.scriptsDir });
  ...
  const configuredProjectsRoot = typeof cfg.projectsRoot === 'string' && cfg.projectsRoot.trim() ? cfg.projectsRoot.trim() : null;
  const projectsRoot = configuredProjectsRoot || defaultProjectsRoot();
  ...
  const registry = makeWorkspaceRegistry({
    ...
    projectsRoot,
    profile,
    scanRoots: configuredProjectsRoot ? [projectsRoot] : [defaultClaudeProjectsRoot(), defaultDshProjectsRoot()],
    memoryRootFor: configuredProjectsRoot ? undefined : (workspacePath) => deriveRoot(workspacePath),
  });
  const sessionAutoWritePath = typeof cfg.sessionAutoWritePath === 'string' && cfg.sessionAutoWritePath.trim()
    ? cfg.sessionAutoWritePath.trim()
    : defaultSessionAutoWritePath();
  const sessionAutoWrite = makeSessionAutoWriteStore({ path: sessionAutoWritePath });
  const pluginConfigPath = typeof cfg.pluginConfigPath === 'string' && cfg.pluginConfigPath.trim()
    ? cfg.pluginConfigPath.trim()
    : defaultPluginConfigPath();
  const pluginConfig = makePluginConfigStore({ path: pluginConfigPath });
  const bootAutoWrite = cfg.autoWrite !== false && cfg.enabled !== false;
  // Mutable runtime state: panel POST /config mutates autoWrite + memoryRootOverride.
  // autoWrite gates idle/goal write-pass reminders only — session-start inject stays on.
  // memoryRootOverride null = use boot config. Resolved at WRITE/SEARCH time.
  // Disk file ~/.dsh/dev-memory/config.json wins over patch.yml for autoWrite after the user saves.
  const savedConfig = pluginConfig.read(bootAutoWrite, bootJev);
  const state = {
    autoWrite: savedConfig.autoWrite,
    jev: savedConfig.jev,
    jevCredential: { configured: false, writable: false },
    workspacePath: null,
  ...
  const memoryInjectedSessions = new Set();
  const memoryInjectionPending = new Set();
  const writePassState = (id) => writePassByAgent.get(id) || { queued: false, fromWritePass: false, skipNextIdle: false, agent: null };
  const writePassActive = (id) => {
    const current = writePassByAgent.get(id);
    return !!(current && (current.queued || current.fromWritePass));
  };
  const rememberAgent = (agent) => {
    const id = sessionKey(agent);
    if (!id) return null;
    const current = writePassState(id);
    writePassByAgent.set(id, { ...current, agent });
    return id;
  };
  const wakeKeptInbox = (agent) => {
    if (!agent) return;
    const inbox = agent.inbox;
    const pending = inbox && (inbox.hasPending || (Array.isArray(inbox.nextTurn) && inbox.nextTurn.length) || (Array.isArray(inbox.nextStep) && inbox.nextStep.length));
    if (!pending) return;
    const kick = () => {
      try {
        const queued = inbox && Array.isArray(inbox.nextTurn) ? inbox.nextTurn.find((row) => row && row.source && row.source.kind === 'user') : null;
        if (queued && typeof agent.send === 'function') {
          if (queued.id && inbox && typeof inbox.remove === 'function') inbox.remove(queued.id);
          agent.send(queued, 'next-turn', true);
          return;
        }
        if (typeof agent.steer === 'function') agent.steer(userMessage('Continue with the queued user message.'));
        else if (typeof agent.followup === 'function') agent.followup(userMessage('Continue with the queued user message.'));
      } catch {
        /* never crash the host while waking a kept inbox */
      }
    };
    if (typeof agent.whenIdle === 'function') {
      Promise.resolve(agent.whenIdle()).then(kick).catch(kick);
      return;
    }
    kick();
  };
  const stopWritePass = (id) => {
    const current = writePassState(id);
    const agent = current.agent;
    if (agent && typeof agent.cancel === 'function') {
      try { agent.cancel({ kind: 'user' }, { keepInbox: true }); } catch { /* cancel is best-effort */ }
    }
    writePassByAgent.set(id, { queued: false, fromWritePass: false, skipNextIdle: true, agent: agent || null });
    wakeKeptInbox(agent);
    return { sessionId: id, writePass: { active: false } };
  };
  ...
  const getCredentials = () => {
    try { return ctx.get(contract.SERVICES.CREDENTIALS); } catch { return null; }
  };
  const describeJevCredential = async () => {
    const ref = state.jev.apiKeyEnv;
    ...
    const configured = typeof process.env[ref] === 'string' && process.env[ref].trim() !== '';
    return { configured, source: configured ? 'environment' : undefined, writable: false };
  };
  const refreshJevCredential = async () => {
    try { state.jevCredential = await describeJevCredential(); }
    catch (err) { remember(err, 'jevCredential'); }
    return state.jevCredential;
  };
  const resolveJevApiKey = async () => {
    const ref = state.jev.apiKeyEnv;
    const credentials = getCredentials();
  ...
  const sessionKey = (agent) => {
    ...
    if (agent.session && typeof agent.session.id === 'string' && agent.session.id) return agent.session.id;
    if (typeof agent.id === 'string' && agent.id) return agent.id;
    return null;
  };
  const parentSessionId = (agent) => {
    const header = agent && agent.session && agent.session.header;
    return header && typeof header.parentSession === 'string' && header.parentSession ? header.parentSession : undefined;
  };
  const autoWriteForAgent = (agent) => {
    const id = sessionKey(agent);
    if (!id) return state.autoWrite;
    try {
      return sessionAutoWrite.resolve(id, state.autoWrite, { parentSessionId: parentSessionId(agent) }).autoWrite;
    } catch {
      return state.autoWrite;
    }
  };
  ...
  const jevService = {
    decide(args) {
      return makeJevService({ resolveApiKey: resolveJevApiKey, model: state.jev.model }).decide(args);
    },
  };
  const gateEnabled = (name) => state.jev.enabled === true && (!state.jev.gates || state.jev.gates[name] !== false);
  const gateService = {
    async decide(args) {
      const startedAt = Date.now();
      const result = await jevService.decide(args);
      try {
        const root = effectiveRoot();
        if (root) appendAudit(jevAuditPath(root), {
          event: 'jev_gate_called',
      ...
      } catch (err) { remember(err, 'jevGateTelemetry'); }
      return result;
    },
  };
  const testJevConnection = async () => {
    const startedAt = Date.now();
    const result = await jevService.decide({
      state: 'Connection test.',
      questions: { connected: { type: 'noul', instructions: 'Does the decision service respond normally?' } },
    });
    if (result && result.ok === true) {
      return { ok: true, model: result.model, latencyMs: Date.now() - startedAt };
    }
    return { ok: false, latencyMs: Date.now() - startedAt, error: result && result.error ? result.error : { category: 'unknown', message: 'Jev connection failed' } };
  };
  const effectiveConfig = () => ({
    ...
    autoWriteLevels,
    writeConfidenceMin,
    enabled: state.autoWrite,
    autoWrite: state.autoWrite,
    jev: { ...state.jev, credential: { ...state.jevCredential } },
  });
  let writePassDispose = null;
  const syncWritePassSection = () => {
    if (writePassDispose) {
      try { writePassDispose(); } catch { /* disposer is best-effort */ }
      writePassDispose = null;
    }
    ...
    try {
      ...
      if (systemPrompt && typeof systemPrompt.section === 'function') {
        ...
        writePassDispose = typeof dispose === 'function' ? dispose : null;
      }
  ...
  if (tools && typeof tools.register === 'function') {
    tools.register({
      ...
      output: { schema: {}, render: jsonRender, presentationMeta: memoryPresentationMeta },
      async execute(args = {}, exec) {
        ...
        const hits = service.search(args.query, top);
        const gated = gateEnabled('retrieval') ? await gateRetrieval({ service: gateService, query: args.query, hits }) : hits;
        return withWorkspace(gated);
      },
    });
    ...
    tools.register({
        ...
        properties: {
          proposal: {
            ...
            properties: {
              ...
              moduleLevel: { type: 'integer', description: 'Optional module level. Conversation memory_write is the confirmation; there is no settings-panel gate.' },
            },
      ...
      async execute(args = {}, exec) {
        ...
        if (gateEnabled('write')) {
          const decision = await gateWrite({ service: gateService, proposal: args.proposal });
          if (!decision.allow) return { written: false, reason: 'Jev write gate chose skip', decision };
        }
    ...
    tools.register({
      ...
      output: { schema: {}, render: healthRender, presentationMeta: memoryPresentationMeta },
      async execute(_args, exec) {
        bindAgentWorkspace(exec && exec.agent);
        const health = service.health();
        const gated = gateEnabled('health') ? await gateHealth({ service: gateService, health }) : health;
        return withWorkspace(gated);
      },
    });
  ...
  const syncJevTool = () => {
    if (jevToolDispose) {
      try { jevToolDispose(); } catch { /* disposer is best-effort */ }
      jevToolDispose = null;
    }
    if (!state.jev.enabled || !tools || typeof tools.register !== 'function') return;
    try {
      const dispose = tools.register(makeJevTool({
        service: jevService,
        onResult(event) {
          try {
            const root = effectiveRoot();
            if (root) appendAudit(jevAuditPath(root), event);
          } catch (err) { remember(err, 'jevTelemetry'); }
        },
      }));
      jevToolDispose = typeof dispose === 'function' ? dispose : null;
    } catch (err) { remember(err, 'jevTool'); }
  };

  // 2) write-pass instruction (the plugin instructs; it never writes directly).
  // Skip when autoWrite is off so idle/goal reminders stay off (tools still register).
  syncWritePassSection();

  let decisionPolicyDispose = null;
  const syncDecisionPolicy = () => {
    if (decisionPolicyDispose) {
      try { decisionPolicyDispose(); } catch { /* disposer is best-effort */ }
      decisionPolicyDispose = null;
    }
    if (!isDecisionPolicyEnabled({ enabled: state.jev.enabled, policyInjection: { enabled: state.jev.policyInjectionEnabled } })) return;
    try {
      const systemPrompt = ctx.get(contract.SERVICES.SYSTEM_PROMPT);
      if (systemPrompt && typeof systemPrompt.section === 'function') {
        const dispose = systemPrompt.section(DECISION_POLICY_SECTION);
        decisionPolicyDispose = typeof dispose === 'function' ? dispose : null;
      }
    } catch (err) { remember(err, 'decisionPolicy'); }
  };
  syncJevTool();
  syncDecisionPolicy();
  void refreshJevCredential();

  // 3) event hooks — intent -> real event mapping (see contract.EVENTS)
  //    "session start -> inject memory"
  try {
  ctx.on(contract.EVENTS.AGENT_SESSION_START, async ({ agent, source } = {}) => {
    bindAgentWorkspace(agent);
    if (!agent) return;
    const header = agent.session && agent.session.header;
    if (source && source !== 'startup') return;
    if (header && (header.isSeeded === true || header.origin === 'subagent' || header.parentSession)) return;
    const id = sessionKey(agent);
    if (!id || memoryInjectedSessions.has(id) || memoryInjectionPending.has(id)) return;
    memoryInjectionPending.add(id);
    let hits;
    try {
      ...
      if (gateEnabled('injection')) hits = await gateRetrieval({ service: gateService, query, hits });
    } catch {
      memoryInjectionPending.delete(id);
      return; // scripts unavailable / empty memory — best effort
    }
    if (resultList(hits).length === 0) {
      memoryInjectedSessions.add(id);
      memoryInjectionPending.delete(id);
      return; // no hits — skip the empty inject
    }
  ...
  ctx.on(contract.EVENTS.AGENT_DISPOSED, ({ agent } = {}) => {
    const id = sessionKey(agent);
    if (!id) return;
    memoryInjectedSessions.delete(id);
    memoryInjectionPending.delete(id);
  });

  //    "goal complete -> write pass" (instruct, not write)
  ctx.on(contract.EVENTS.GOAL_CHANGED, ({ agent, change } = {}) => {
    if (!autoWriteForAgent(agent)) return;
  ...
  ctx.on(contract.EVENTS.AGENT_STATUS, ({ agent, status } = {}) => {
    ...
    if (!id || !autoWriteForAgent(agent)) return;
    const current = writePassState(id);
    if (status === 'running') {
      if (!current.fromWritePass) writePassByAgent.set(id, { queued: false, fromWritePass: false, skipNextIdle: false, agent });
      else writePassByAgent.set(id, { ...current, agent });
      return;
    }
    if (status !== 'idle') return;
    if (current.skipNextIdle) {
      writePassByAgent.set(id, { queued: false, fromWritePass: false, skipNextIdle: false, agent });
      return;
    }
    ...
    if (followupText(agent, BOUNDARY_REMINDER)) writePassByAgent.set(id, { queued: true, fromWritePass: true, skipNextIdle: false, agent });
  });

  ctx.on(contract.EVENTS.AGENT_INBOX_INSERTED, ({ agent, message } = {}) => {
    const id = rememberAgent(agent);
    if (!id || !writePassActive(id)) return;
    const kind = message && message.source && message.source.kind;
    if (kind !== 'user') return;
    stopWritePass(id);
  });
    ...
    ctx.inject(['webServer'], (host) => {
      ...
      const mount = () => mountDevMemoryRoutes(host.webServer, {
        ...
        async updateConfig(body) {
          ...
          if (typeof payload.autoWrite === 'boolean') {
            state.autoWrite = payload.autoWrite;
            try { pluginConfig.set({ autoWrite: state.autoWrite }); } catch { /* persist is best-effort */ }
            syncWritePassSection();
          } else if (typeof payload.enabled === 'boolean') {
            state.autoWrite = payload.enabled;
            try { pluginConfig.set({ autoWrite: state.autoWrite }); } catch { /* persist is best-effort */ }
            syncWritePassSection();
          }
          ...
          if (payload.jev && typeof payload.jev === 'object' && !Array.isArray(payload.jev)) {
            const patch = payload.jev;
            if (typeof patch.enabled === 'boolean') state.jev.enabled = patch.enabled;
            if (typeof patch.policyInjectionEnabled === 'boolean') state.jev.policyInjectionEnabled = patch.policyInjectionEnabled;
            if (typeof patch.model === 'string' && patch.model.trim()) state.jev.model = patch.model.trim();
            if (typeof patch.apiKeyEnv === 'string' && patch.apiKeyEnv.trim()) {
              const ref = patch.apiKeyEnv.trim();
              if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(ref)) {
                const err = new Error('jev.apiKeyEnv must be a valid credential reference');
                err.status = 400;
                throw err;
              }
              state.jev.apiKeyEnv = ref;
            }
            const credentials = getCredentials();
            if (typeof patch.apiKey === 'string' && patch.apiKey.trim()) {
              if (!credentials || typeof credentials.set !== 'function') {
                const err = new Error('DSH credentials service is unavailable');
                err.status = 503;
                throw err;
              }
              await credentials.set(state.jev.apiKeyEnv, patch.apiKey.trim());
            }
            if (patch.clearApiKey === true) {
              ...
              await credentials.unset(state.jev.apiKeyEnv);
            }
            pluginConfig.set({ jev: state.jev });
            await refreshJevCredential();
            syncJevTool();
            syncDecisionPolicy();
          }
        ...
        getSessionAutoWrite(request) {
          ...
          const resolved = sessionAutoWrite.resolve(sessionId, state.autoWrite);
          return { ...resolved, writePass: { active: writePassActive(sessionId) } };
        },
        setSessionAutoWrite(body) {
          const payload = body && typeof body === 'object' ? body : {};
          ...
          if (payload.action === 'stopWritePass') {
            const stopped = stopWritePass(sessionId);
            const resolved = sessionAutoWrite.resolve(sessionId, state.autoWrite);
            return { ...resolved, ...stopped, writePass: { active: false } };
          }
          if (typeof payload.autoWrite !== 'boolean') {
            const err = new Error('autoWrite must be a boolean');
            err.status = 400;
            throw err;
          }
          return { ...sessionAutoWrite.set(sessionId, payload.autoWrite), writePass: { active: writePassActive(sessionId) } };
        },
        testJevConnection,
        listWorkspaces() {
