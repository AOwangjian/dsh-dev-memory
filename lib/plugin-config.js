...
function normalize(raw) {
  ...
  const jev = raw && typeof raw === 'object' ? normalizeJev(raw.jev) : undefined;
  if (jev) out.jev = jev;
  return out;
}

function normalizeJev(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return undefined;
  const out = {};
  if (typeof raw.enabled === 'boolean') out.enabled = raw.enabled;
  if (typeof raw.policyInjectionEnabled === 'boolean') out.policyInjectionEnabled = raw.policyInjectionEnabled;
  if (typeof raw.apiKeyEnv === 'string' && CREDENTIAL_REF.test(raw.apiKeyEnv)) out.apiKeyEnv = raw.apiKeyEnv;
...
export function makePluginConfigStore({ path }) {
  ...
  return {
    read(fallbackAutoWrite, fallbackJev = {}) {
      const data = readLatest();
      return {
        autoWrite: typeof data.autoWrite === 'boolean' ? data.autoWrite : fallbackAutoWrite === true,
        jev: { ...normalizeJev(fallbackJev), ...normalizeJev(data.jev) },
      };
    },
    set(patch = {}) {
      acquire();
      try {
        ...
        const jev = normalizeJev(patch.jev);
        if (jev) current.jev = { ...normalizeJev(current.jev), ...jev };
        current.version = VERSION;
        atomicWrite(current);
        return {
          autoWrite: current.autoWrite,
          jev: { ...normalizeJev(current.jev) },
        };
      } finally { release(); }
    },
  };
