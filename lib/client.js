...
function formatIssueValue(value) {
  ...
  const primary = value.file || value.path || value.relPath || value.dir || value.module || '';
  const detail = value.meta || value.reason || value.message || value.issue || '';
  if (primary && detail) return String(primary) + ' — ' + String(detail);
  if (primary || detail) return String(primary || detail);
...
function createClientPlugin(React) {
  ...
  function LivePanel() {
    ...
    const hs = (health && health.summary) || {};
    const hi = (health && health.issues) || {};
    const sev = hs.severityCounts || {};
    const issueKeys = Object.keys(hi).filter(function (k) { return Array.isArray(hi[k]) && hi[k].length > 0; });
    ...
    return h('div', { 'data-dev-memory': 'review-panel', style: PANEL_STYLE },
      ...
      h('div', { style: ROW_STYLE },
        ...
        h('button', {
          type: 'button',
          style: BTN_STYLE,
          disabled: busy || cfg.rootMode === 'auto' || cfg.rootMode === 'auto-waiting',
          onClick: function () { postConfig({ memoryRoot: '' }); },
        }, 'Use Auto'),
      ),
      h('div', { style: { ...MUTED_STYLE, marginTop: '-4px', marginBottom: '8px' } },
        (cfg.rootMode === 'auto-waiting'
          ? 'Waiting for active session workspace'
          : (cfg.rootMode === 'auto' ? 'Auto (session cwd)' : 'Configured')) +
        (cfg.workspacePath ? ' · workspace: ' + cfg.workspacePath : ''),
      ),
      ...
      h('div', { style: { ...MUTED_STYLE, marginTop: '8px' } },
        health == null
          ? 'health: (loading)'
          : (health.error ? ('health: error — ' + health.error) : ('health: ok · dirs ' + (hs.directories || 0) + ' · files ' + (hs.markdownFiles || 0) + ' · index ' + (hs.memoryIndexExists ? 'yes' : 'no') + ' · sev ' + ((sev.high || 0) + '/' + (sev.medium || 0) + '/' + (sev.low || 0)))),
        issueKeys.length === 0
          ? null
          : h('ul', { style: { ...MUTED_STYLE, paddingLeft: '18px', margin: '2px 0', fontSize: '12px' } },
              issueKeys.map(function (k) {
                const arr = hi[k];
                return h('li', { key: k }, k + ': ' + arr.length + (arr.length ? ' (' + arr.slice(0, 3).map(formatIssueValue).join(', ') + (arr.length > 3 ? ' …' : '') + ')' : ''));
              })),
      ),
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { createClientPlugin, formatIssueValue, default: createClientPlugin };
}
