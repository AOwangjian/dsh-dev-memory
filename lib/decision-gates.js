...
export async function gateRetrieval({ service, query, hits }) {
  ...
  if (!service || typeof service.decide !== 'function' || candidates.length === 0) {
    return { ...(Array.isArray(hits) ? { results: hits } : hits), decision: { gated: false, candidateCount: candidates.length, kept: candidates.length, fallback: true } };
  }
  ...
  try {
    ...
    return {
      ...
      decision: { gated: true, candidateCount: candidates.length, kept: kept.length || candidates.length, fallback: kept.length === 0 },
    };
  } catch {
    return { ...(Array.isArray(hits) ? {} : hits), results: candidates, decision: { gated: false, candidateCount: candidates.length, kept: candidates.length, fallback: true } };
  }
}
